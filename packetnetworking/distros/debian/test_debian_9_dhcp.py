...
@pytest.fixture
def debian_9_dhcp_network(generic_debian_dhcp_network):
    def _builder(**kwargs):
        return generic_debian_dhcp_network("debian", "9", **kwargs)

    return _builder


def test_debian_9_dhcp_task_etc_network_interfaces(
    debian_9_dhcp_network
):
    """Validates /etc/network/interfaces for dhcp"""

    builder = debian_9_dhcp_network(public=True)
...
def test_debian_9_etc_resolvers_configured(debian_9_dhcp_network, fake):
    """
    Validates /etc/resolv.conf is configured correctly
    """
    builder = debian_9_dhcp_network()
    resolver1 = fake.ipv4()
    resolver2 = fake.ipv4()
...
def test_debian_9_etc_hostname_configured(debian_9_dhcp_network):
    """
...
def test_debian_9_etc_hosts_configured(debian_9_dhcp_network):
    """
...
def test_debian_9_no_persistent_interface_names(debian_9_dhcp_network):
    """
